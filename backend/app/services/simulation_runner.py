...
logger = get_logger('nexus.simulation_runner')
...
class SimulationRunner:
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            logger.error(f"加载运行状态失败: {state_file}, {str(e)}")
            recovered_state = cls._recover_corrupted_run_state(simulation_id, state_file)
            if recovered_state:
                logger.warning(f"已自动恢复损坏的运行状态文件: {simulation_id}")
    ...
    @classmethod
    def _extract_string_field(cls, text: str, field_name: str) -> Optional[str]:
    ...
    @classmethod
    def _extract_int_field(cls, text: str, field_name: str, default: int = 0) -> int:
        match = re.search(rf'"{re.escape(field_name)}"\s*:\s*(-?\d+)', text)
        return int(match.group(1)) if match else default

    @classmethod
    def _extract_bool_field(cls, text: str, field_name: str, default: bool = False) -> bool:
    ...
    @classmethod
    def _pid_is_alive(cls, pid: Optional[int]) -> bool:
        if not pid:
            return False
        try:
            os.kill(pid, 0)
    ...
    @classmethod
    def _recover_corrupted_run_state(
        cls,
        simulation_id: str,
        state_file: str
    ) -> Optional[SimulationRunState]:
        """从损坏的 run_state.json 中恢复可用的精简状态，并隔离原文件"""
        try:
            with open(state_file, 'r', encoding='utf-8') as f:
                raw_text = f.read()
        except OSError as e:
            logger.error(f"读取损坏运行状态文件失败: {state_file}, {e}")
            return None

        runner_status_text = cls._extract_string_field(raw_text, 'runner_status') or RunnerStatus.IDLE.value
        process_pid = cls._extract_int_field(raw_text, 'process_pid', 0) or None

        if runner_status_text in {
            RunnerStatus.RUNNING.value,
            RunnerStatus.STARTING.value,
            RunnerStatus.PAUSED.value,
            RunnerStatus.STOPPING.value,
        } and not cls._pid_is_alive(process_pid):
            runner_status_text = RunnerStatus.STOPPED.value

        try:
            runner_status = RunnerStatus(runner_status_text)
        except ValueError:
        ...
        state = SimulationRunState(
            ...
            current_round=cls._extract_int_field(raw_text, 'current_round', 0),
            total_rounds=cls._extract_int_field(raw_text, 'total_rounds', 0),
            simulated_hours=cls._extract_int_field(raw_text, 'simulated_hours', 0),
            total_simulation_hours=cls._extract_int_field(raw_text, 'total_simulation_hours', 0),
            twitter_current_round=cls._extract_int_field(raw_text, 'twitter_current_round', 0),
            reddit_current_round=cls._extract_int_field(raw_text, 'reddit_current_round', 0),
            twitter_simulated_hours=cls._extract_int_field(raw_text, 'twitter_simulated_hours', 0),
            reddit_simulated_hours=cls._extract_int_field(raw_text, 'reddit_simulated_hours', 0),
            twitter_running=cls._extract_bool_field(raw_text, 'twitter_running', False),
            reddit_running=cls._extract_bool_field(raw_text, 'reddit_running', False),
            twitter_completed=cls._extract_bool_field(raw_text, 'twitter_completed', False),
            reddit_completed=cls._extract_bool_field(raw_text, 'reddit_completed', False),
            twitter_actions_count=cls._extract_int_field(raw_text, 'twitter_actions_count', 0),
            reddit_actions_count=cls._extract_int_field(raw_text, 'reddit_actions_count', 0),
            started_at=cls._extract_string_field(raw_text, 'started_at'),
            updated_at=cls._extract_string_field(raw_text, 'updated_at') or datetime.now().isoformat(),
            completed_at=cls._extract_string_field(raw_text, 'completed_at'),
            error='run_state.json 已损坏，系统已自动恢复为精简状态（recent_actions 已清空）',
            process_pid=process_pid,
        )

        backup_path = f"{state_file}.corrupt.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        try:
            shutil.copy2(state_file, backup_path)
        except OSError as e:
            logger.warning(f"备份损坏运行状态文件失败: {state_file}, {e}")

        cls._save_run_state(state)
        logger.warning(f"损坏的运行状态文件已备份到: {backup_path}")
        return state
