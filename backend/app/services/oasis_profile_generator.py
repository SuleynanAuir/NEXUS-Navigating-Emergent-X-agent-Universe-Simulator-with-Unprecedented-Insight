...
logger = get_logger('nexus.oasis_profile')
...
class OasisProfileGenerator:
    ...
    # 单批次MBTI分布上限（同一类型最多占比）
    MBTI_MAX_RATIO_PER_BATCH = 0.2

    SOCIAL_TONE_GUIDELINE = (
        "写作风格要求：像真实社媒用户发言，短句+口语为主，"
        "可以有情绪但不失逻辑；避免官方声明、公文套话、模板化表达；"
        "尽量结合具体人物/事件细节，不要空泛复读。"
    )
    ...
    def _normalize_mbti(self, mbti: Optional[str]) -> Optional[str]:
        ...
        if not mbti:
            return None
        normalized = str(mbti).strip().upper()
        return normalized if normalized in self.MBTI_TYPES else None

    def _pick_stable_mbti(self, entity_name: str, entity_type: str, entity_uuid: Optional[str] = None) -> str:
        """基于实体信息稳定选择MBTI，避免批量同质化。"""
        import hashlib

        key = f"{entity_uuid or ''}|{entity_type}|{entity_name}"
        digest = hashlib.md5(key.encode("utf-8")).hexdigest()
        idx = int(digest[:8], 16)

        pool = self._get_mbti_pool_for_entity_type(entity_type)

        return pool[idx % len(pool)]

    def _get_mbti_pool_for_entity_type(self, entity_type: str) -> List[str]:
        """按实体类型返回更合理的MBTI候选池。"""
        entity_type_lower = (entity_type or "").lower()
        if entity_type_lower in ["student", "alumni"]:
            return ["ENFP", "ESFP", "INFP", "ISFP", "ENTP", "ENFJ", "ISFJ", "ESTP"]
        if entity_type_lower in ["publicfigure", "expert", "faculty", "official", "journalist"]:
            return ["ENTJ", "INTJ", "ENTP", "INTP", "ENFJ", "INFJ", "ESTJ", "ISTJ"]
        if entity_type_lower in ["mediaoutlet", "socialmediaplatform", "university", "governmentagency", "ngo", "organization"]:
            return ["ISTJ", "ESTJ", "INTJ", "ENTJ", "ENFJ", "INFJ"]
        return self.MBTI_TYPES

    def _enforce_mbti_quota(
        self,
        profiles: List[Optional[OasisAgentProfile]],
        max_ratio: Optional[float] = None
    ) -> None:
        """对单批次Profile执行MBTI配额约束，避免同一类型过度集中。"""
        import hashlib
        import math
        ...
        ratio = max_ratio if max_ratio is not None else self.MBTI_MAX_RATIO_PER_BATCH
        ratio = min(max(ratio, 0.05), 1.0)
        total = len(valid_profiles)
        max_per_type = max(1, math.ceil(total * ratio))

        for profile in valid_profiles:
            profile.mbti = self._normalize_mbti(profile.mbti) or self._pick_stable_mbti(
                entity_name=profile.name,
                entity_type=profile.source_entity_type or "Entity",
                entity_uuid=profile.source_entity_uuid
            )

        counts = Counter(profile.mbti for profile in valid_profiles if profile.mbti)
        if not any(count > max_per_type for count in counts.values()):
            return

        adjusted_count = 0
        over_types = [mbti for mbti, count in counts.items() if count > max_per_type]

        for over_mbti in over_types:
            while counts[over_mbti] > max_per_type:
                candidates = [
                    profile for profile in valid_profiles
                    if profile.mbti == over_mbti
                ]
                if not candidates:
                    break
                ...
                target_profile = sorted(
                    candidates,
                    key=lambda profile: (
                        profile.user_id if profile.user_id is not None else 0,
                ...
                preferred_pool = self._get_mbti_pool_for_entity_type(target_profile.source_entity_type or "Entity")
                under_quota_preferred = [m for m in preferred_pool if m != over_mbti and counts[m] < max_per_type]
                under_quota_global = [m for m in self.MBTI_TYPES if m != over_mbti and counts[m] < max_per_type]

                replacement_pool = under_quota_preferred or under_quota_global
                if not replacement_pool:
                    break

                hash_key = f"{target_profile.source_entity_uuid or ''}|{target_profile.name}|{target_profile.user_id}"
                digest = hashlib.md5(hash_key.encode("utf-8")).hexdigest()
                replacement_idx = int(digest[:8], 16) % len(replacement_pool)
                new_mbti = replacement_pool[replacement_idx]

                target_profile.mbti = new_mbti
                counts[over_mbti] -= 1
                counts[new_mbti] += 1
                adjusted_count += 1
        ...
        if adjusted_count > 0:
            logger.info(
                f"MBTI配额重平衡完成: 调整 {adjusted_count} 个Profile, "
                f"max_per_type={max_per_type}/{total} (ratio={ratio:.2f})"
            )

    def _resolve_profile_mbti(
        self,
        ...
        """优先使用LLM输出，失败则按实体稳定分配MBTI。"""
        normalized = self._normalize_mbti(profile_data.get("mbti"))
        if normalized:
            return normalized
        return self._pick_stable_mbti(entity_name=entity_name, entity_type=entity_type, entity_uuid=entity_uuid)
    ...
    def generate_profile_from_entity(
        ...
        resolved_mbti = self._resolve_profile_mbti(
...
- persona中要明确此人对当前事件的关注点和利益相关位置（为何在意这件事）
- 语言风格偏社媒口语，避免官腔与空话，允许有真实情绪起伏
- 给出2-3个该角色常见表达习惯（例如常用词、语气、句式），减少所有人说话一个样
"""
...
- 机构账号发言要符合其身份定位
- 虽然是机构账号，但表达应接近真实社媒运营语气，避免官样文章
- 明确该机构在事件中的目标受众、风险顾虑与优先回应点，提升针对性"""

    def _apply_social_tone_guideline(self, persona: str) -> str:
        """在 persona 末尾附加统一的社媒表达约束，减少官腔和复读。"""
        if not persona:
            return self.SOCIAL_TONE_GUIDELINE
        if self.SOCIAL_TONE_GUIDELINE in persona:
            return persona
        return f"{persona} {self.SOCIAL_TONE_GUIDELINE}"
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": self._pick_stable_mbti(entity_name, entity_type),
                "country": "中国",
                "profession": "Media",
    ...
    def generate_profiles_from_entities(
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                fallback_profile = OasisAgentProfile(
                    ...
                    mbti=self._pick_stable_mbti(entity.name, entity_type, entity.uuid),
                    source_entity_uuid=entity.uuid,
                    source_entity_type=entity_type,
                )
        ...
        # 批量约束：同一MBTI占比不超过阈值（默认20%）
        self._enforce_mbti_quota(profiles)
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                if profile.persona and profile.persona != profile.bio:
                    user_char = f"{profile.bio} {self._apply_social_tone_guideline(profile.persona)}"
                else:
                    user_char = f"{profile.bio} {self.SOCIAL_TONE_GUIDELINE}"
                # 处理换行符（CSV中用空格替代）
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                "persona": self._apply_social_tone_guideline(
                ...
                "mbti": self._normalize_mbti(profile.mbti) or self._pick_stable_mbti(
                    profile.name,
                    profile.source_entity_type or "Entity",
