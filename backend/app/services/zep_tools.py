...
logger = get_logger('nexus.zep_tools')


class ZepToolRateLimitError(Exception):
    """Zep 限流/配额错误（工具层）"""
...
class ZepToolsService:
    ...
    def _is_rate_limit_error(self, info: Dict[str, Any]) -> bool:
        status_code = info.get("status_code")
        body_lower = (info.get("body_text") or "").lower()
        return status_code == 429 or "rate limit" in body_lower

    def _is_retryable_api_error(self, info: Dict[str, Any]) -> bool:
        status_code = info.get("status_code")
        return status_code in {408, 409, 425, 429, 500, 502, 503, 504}

    def _format_rate_limit_error(self, info: Dict[str, Any], operation_name: str) -> str:
        retry_after = info.get("retry_after")
        rate_reset = info.get("rate_reset")
        extra = ""
        if retry_after:
            extra += f" 建议至少等待 {retry_after} 秒后重试。"
        if rate_reset:
            extra += f" 配额重置时间戳: {rate_reset}。"
        return (
            f"Zep 请求受限（{operation_name}）：当前账号触发 FREE plan 速率限制。"
            f"{extra}"
            "请稍后重试或升级/更换 ZEP_API_KEY。"
        )
    ...
    def _call_with_retry(self, func, operation_name: str, max_retries: int = None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                is_rate_limit = self._is_rate_limit_error(info)
                ...
                if attempt < max_retries - 1 and is_retryable:
                    ...
                    logger.warning(
                        f"Zep {operation_name} 第 {attempt + 1}/{max_retries} 次失败 "
                        f"(status={info.get('status_code')}), {wait_seconds:.1f}秒后重试..."
                    )
                ...
                if is_rate_limit:
                    raise ZepToolRateLimitError(self._format_rate_limit_error(info, operation_name)) from e
        ...
        raise RuntimeError(f"Zep {operation_name} 调用失败")
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        try:
            nodes = self._call_with_retry(
                func=lambda: fetch_all_nodes(self.client, graph_id),
                operation_name=f"获取全部节点(graph={graph_id})"
            )
        except Exception as e:
            logger.error(f"获取全部节点失败: {str(e)}")
            return []
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        try:
            edges = self._call_with_retry(
                func=lambda: fetch_all_edges(self.client, graph_id),
                operation_name=f"获取全部边(graph={graph_id})"
            )
        except Exception as e:
            logger.error(f"获取全部边失败: {str(e)}")
            return []
...
返回JSON格式的子问题列表。"""
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "7. 每个问题至少给出1个具体细节（人物/事件/动作/时间），不要泛泛而谈\n"
            "8. 不要复述前一个问题的同一句话，必须新增信息或新角度\n"
            "9. 语言风格保持社媒口语化，不要官方公告腔\n\n"
        )
