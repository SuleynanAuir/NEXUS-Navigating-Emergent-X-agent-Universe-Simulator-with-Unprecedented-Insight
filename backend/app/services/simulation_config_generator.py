...
logger = get_logger('nexus.simulation_config')
...
class SimulationConfigGenerator:
    ...
    def _build_context(
        ...
        if focus_entities or focus_events:
            prior_parts = []
            if focus_entities:
                prior_parts.append(f"**特别关注的人物**：{focus_entities}")
            if focus_events:
                prior_parts.append(f"**特别关注的事件**：{focus_events}")
            if prior_parts:
                context_parts.append(f"\n## 分析先验设置\n" + "\n".join(prior_parts) + "\n\n请在生成模拟配置时，对这些关注对象和事件进行更重要的分析和更高维度的剖析。")
