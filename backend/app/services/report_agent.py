...
logger = get_logger('nexus.report_agent')
...
class ReportAgent:
    ...
    def generate_report(
        ...
        try:
            ...
            # 主流程失败后，自动生成应急报告兜底，确保流程可完成
            try:
                report = self._build_emergency_report(
                    report=report,
                    report_id=report_id,
                    original_error=str(e),
                    completed_section_titles=completed_section_titles
                )
                logger.warning(f"主流程失败，已生成应急报告兜底: {report_id}")
            except Exception as fallback_error:
                logger.error(f"应急报告兜底失败: {str(fallback_error)}")
                report.status = ReportStatus.FAILED
                report.error = f"{str(e)} | fallback_failed: {str(fallback_error)}"
    ...
    def _build_emergency_report(
        ...
        """构建本地应急报告（不依赖Zep/LLM），确保报告流程可完成"""
        simulation_dir = os.path.join(Config.OASIS_SIMULATION_DATA_DIR, self.simulation_id)

        state_data = self._safe_read_json_file(os.path.join(simulation_dir, 'state.json'))
        run_state_data = self._safe_read_json_file(os.path.join(simulation_dir, 'run_state.json'))
        sim_config_data = self._safe_read_json_file(os.path.join(simulation_dir, 'simulation_config.json'))

        # 提取关键指标（尽量从本地文件恢复）
        status = run_state_data.get('runner_status') or state_data.get('status') or 'unknown'
        current_round = run_state_data.get('current_round', 0)
        total_rounds = run_state_data.get('total_rounds', 0)
        total_hours = (
            run_state_data.get('total_simulation_hours')
            or (sim_config_data.get('time_config') or {}).get('total_simulation_hours')
            or 0
        )
        twitter_actions = run_state_data.get('twitter_actions_count', 0)
        reddit_actions = run_state_data.get('reddit_actions_count', 0)
        entities_count = state_data.get('entities_count', 0)
        profiles_count = state_data.get('profiles_count', 0)
        entity_types = state_data.get('entity_types', [])

        time_config = sim_config_data.get('time_config') or {}
        event_config = sim_config_data.get('event_config') or {}

        section1 = ReportSection(
            title='应急执行摘要',
            content=(
                f"本报告由系统自动兜底生成，用于保证报告流程可用性。\n\n"
                f"- 模拟ID：`{self.simulation_id}`\n"
                f"- 图谱ID：`{self.graph_id}`\n"
                f"- 模拟状态：`{status}`\n"
                f"- 已执行轮次：`{current_round}` / `{total_rounds}`\n"
                f"- 累计动作数：Twitter `{twitter_actions}`，Reddit `{reddit_actions}`\n"
                f"- 实体/人设规模：`{entities_count}` / `{profiles_count}`\n"
                f"- 失败原因：{original_error}\n\n"
                f"说明：主流程因外部服务（例如限流/网络）失败，系统已自动降级为本地数据报告。"
            )
        )
        ...
        section2 = ReportSection(
            title='本地数据快照',
            content=(
                f"**模拟需求**\n\n{self.simulation_requirement or '（未提供）'}\n\n"
                f"**时间配置**\n\n"
                f"- 总模拟时长（小时）：`{total_hours}`\n"
                f"- 每轮分钟数：`{time_config.get('minutes_per_round', 'unknown')}`\n"
                f"- 峰值时段：`{time_config.get('peak_hours', [])}`\n"
                f"- 低谷时段：`{time_config.get('off_peak_hours', [])}`\n\n"
                f"**事件配置摘要**\n\n"
                f"- 热点话题数：`{len(event_config.get('hot_topics', []) if isinstance(event_config.get('hot_topics', []), list) else [])}`\n"
                f"- 舆论方向数：`{len(event_config.get('opinion_directions', []) if isinstance(event_config.get('opinion_directions', []), list) else [])}`\n"
                f"- 实体类型：`{entity_types}`"
            )
        )
        ...
        emergency_outline = ReportOutline(
            title='模拟分析报告（应急兜底版）',
            summary='主流程失败后自动生成的本地数据报告，用于保证报告链路可用。',
            sections=[section1, section2, section3]
        )

        # 保存应急大纲与章节
        ReportManager.save_outline(report_id, emergency_outline)
        for idx, section in enumerate(emergency_outline.sections, start=1):
            ReportManager.save_section(report_id, idx, section)

        # 组装完整报告并标记完成
        report.outline = emergency_outline
        report.markdown_content = ReportManager.assemble_full_report(report_id, emergency_outline)
        report.status = ReportStatus.COMPLETED
        report.completed_at = datetime.now().isoformat()
        report.error = (
            f"主流程失败已自动兜底: {original_error}"
        )
        ...
        ReportManager.update_progress(
            ...
            'completed',
            100,
            '主流程失败，已自动生成应急报告（可正常查看）',
            completed_sections=completed_section_titles if completed_section_titles else [s.title for s in emergency_outline.sections]
        )

        if self.report_logger:
            self.report_logger.log(
                action='fallback_report_complete',
                stage='completed',
                details={
                    'message': '主流程失败后已生成应急报告',
                    'original_error': original_error,
                    'sections': [s.title for s in emergency_outline.sections]
                }
            )
