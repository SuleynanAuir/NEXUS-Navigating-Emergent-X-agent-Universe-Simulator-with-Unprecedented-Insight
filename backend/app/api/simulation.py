...
logger = get_logger('nexus.api.simulation')
...
INTERVIEW_PROMPT_REQUIREMENTS = (
    "回复要求：\n"
    "1. 先给明确立场，再给依据，最后给结论或下一步判断；\n"
    "2. 必须引用至少1个你记忆里的具体细节（人物/事件/时间/动作），禁止空泛套话；\n"
    "3. 避免复读你之前说过的原句，和历史回答至少在角度或细节上有新增；\n"
    "4. 使用社交媒体自然口吻，允许口语短句，不要公文腔、新闻稿腔；\n"
    "5. 不要返回JSON、Markdown标题或工具调用格式。"
)

INTERVIEW_PROMPT_REQUIREMENTS_PERSONAL = (
    "回复要求：\n"
    "1. 用第一人称，像真实个人在说真心话，允许有犹豫、情绪和纠结；\n"
    "2. 必须结合你自己的经历或记忆细节（人物/事件/动作/时间）来回答；\n"
    "3. 不要官话、套话、新闻稿腔，语言自然口语化；\n"
    "4. 给出真实动机（你为什么在意、担心什么、希望什么）；\n"
    "5. 不要返回JSON、Markdown标题或工具调用格式。"
)


def optimize_interview_prompt(prompt: str, response_style: str = "official") -> str:
    """
    优化Interview提问，添加前缀避免Agent调用工具
    
    Args:
        prompt: 原始提问
        response_style: 回答风格（official/personal）
    ...
    requirements = (
        ...
        else INTERVIEW_PROMPT_REQUIREMENTS
    )

    return f"{INTERVIEW_PROMPT_PREFIX}\n{requirements}\n\n用户问题：{prompt}"
    ...
    请求（JSON）：
        {
            ...
            "graph_id": "nexus_xxxx",    // 可选，如不提供则从project获取
...
def interview_agent():
    ...
    try:
        ...
        response_style = data.get('response_style', 'official')
        ...
        if response_style not in ("official", "personal"):
            return jsonify({
                "success": False,
                "error": "response_style 参数只能是 'official' 或 'personal'"
            }), 400
        ...
        optimized_prompt = optimize_interview_prompt(prompt, response_style=response_style)
...
def interview_agents_batch():
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            item_style = interview.get('response_style')
            if item_style and item_style not in ("official", "personal"):
                return jsonify({
                    "success": False,
                    "error": f"采访列表第{i+1}项的response_style只能是 'official' 或 'personal'"
                }), 400
        ...
        for interview in interviews:
            ...
            item_style = interview.get('response_style', response_style)
            optimized_interview['prompt'] = optimize_interview_prompt(
                interview.get('prompt', ''),
                response_style=item_style
            )
