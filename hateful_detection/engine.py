...
class CLIPClassifier(pl.LightningModule):
    ...
    def __init__(self, args, fine_grained_labels, compute_fine_grained_metrics):
        ...
        # 修复 torchmetrics 新版本兼容性
        if self.dataset == 'prop':
            self.acc = torchmetrics.Accuracy(task='multiclass', num_classes=22)
            self.auroc = torchmetrics.AUROC(task='multiclass', num_classes=22)
            self.precision_score = torchmetrics.Precision(task='multiclass', num_classes=22, average='macro')
            self.recall = torchmetrics.Recall(task='multiclass', num_classes=22, average='macro')
            self.f1 = torchmetrics.F1Score(task='multiclass', num_classes=22, average='macro')
        else:
        ...
        if args.local_pretrained_weights != 'none':
            loaded = torch.load(args.local_pretrained_weights)
            # 如果是 Lightning checkpoint，取其中的 state_dict
            if isinstance(loaded, dict) and 'state_dict' in loaded:
                candidate_sd = loaded['state_dict']
            else:
                candidate_sd = loaded
            # 只筛选属于 clip.* 的键，或已经去掉前缀的 visual/text projection（兼容未来格式）
            clip_sd = {}
            for k, v in candidate_sd.items():
                if k.startswith('clip.'):
                    clip_sd[k[len('clip.'):]] = v
                elif k in ('visual_projection.weight','visual_projection.bias','text_projection.weight','text_projection.bias'):
                    clip_sd[k] = v
            missing, unexpected = self.clip.load_state_dict(clip_sd, strict=False)
            print(f"[load_local_pretrained] loaded {len(clip_sd)} keys; missing={len(missing)}, unexpected={len(unexpected)}")
